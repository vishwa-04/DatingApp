...
export const isSignedIn = async (): Promise<boolean> => {
    try {
      ...
      return authToken.resType === 'SUCCESS' && authToken.data?.is_profile_updated === true;
    } catch (error) {
      return false;
    }
  };

export const setAuthToken = async (loginData: AuthToken): Promise<AuthResponse> => {
  try {
...
export const getAuthToken = async (): Promise<AuthResponse> => {
  try {
    ...
    if (authToken) {
      ...
      return { resType: 'SUCCESS', data: parsedToken };
    } else {
...
export const setUserProfilingDone = async (status: boolean): Promise<AuthResponse> => {
  try {
    const authToken = await getAuthToken();
